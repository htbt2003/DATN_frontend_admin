...
const ProductAttribute = ({ optionAttrs, setOptionAttrs, attributes }) => {
  ...
  const handleValueChange = (indexOp, indexVal, e) => {
    const newOptions = [...optionAttrs];
    newOptions[indexOp].values[indexVal] = { ...newOptions[indexOp].values[indexVal], attribute_value_id: e.target.value };
    setOptionAttrs(newOptions);
  };
  ...
  return (
    <div className="bg-white p-5 rounded-xl mt-3">
      ...
      <div className="mt-5 bg-blue-500 text-white p-2 rounded w-[150px]" onClick={addOption}>+ Thêm thuộc tính</div>
    </div>
  );
};

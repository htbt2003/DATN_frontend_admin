...
function ContactList() {
  ...
  var condition = {
    ...
    useEffect(function(){
      (async function(){
        setLoad(true)
        const result = await ContactServices.getAll(page, condition);
    ...
    function itemDelete(id) {
      swal({
        ...
        if (result) {
          try {
            const result = await ContactServices.delete(id)
            setReLoad(id)
          }
          catch (error) {
            console.log(error)
    ...
    function handleStatus(item) {
      swal({
        ...
        if (result) {
          try {
            const result = await ContactServices.changeStatus(item.id)
            setReLoad(Date.now)
          }
          catch (error) {
            console.log(error)
    ...
    const handleAction = () => {
      ...
      swal({
        ...
        if (result) {
          if (action === "trash") {
            try {
              const result = await ContactServices.action_trash(data);
              setReLoad(Date.now)
              console.log(true)
            }
            catch (error) {

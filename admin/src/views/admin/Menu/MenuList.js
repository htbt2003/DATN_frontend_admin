...
function MenuList() {
  ...
  useEffect(function () {
    (async function () {
      try {
        setLoad(true)
        const result = await MenuServices.getAll(page);
        setTotal(result.total)
        setPublish(result.publish)
        setTrash(result.trash)
        setMenus(result.menus.data)
        setCategoryies(result.categoryies)
  ...
  async function handleStatus(id) {
    const result = await MenuServices.changeStatus(id)
    // alert(result.message)
    setReLoad(Date.now)
  }
    ...
    <div>
      ...
      <div className="page-header">
        <div className='row'>
          ...
          <div className="mr-2">
            <select name="" className="border border-dark rounded" value={filter} onChange={(e) => setFilter(e.target.value)}>
          ...
          <div onClick={() => handleFilter(filter)} className=" p-2 mr-4 btn-secondary" style={{ height: 35, background: "#bfbfbf", border: "none", borderRadius: 2, color: "black", textAlign: "center" }}>Lọc</div>
        </div>
                  ...
                  <tbody>
                    {
                      filter == 0 && key == 0 ?
                        ...
                        :
                        (key != 0 ?
                          (search_data && search_data.length > 0 && search_data.map(function (menu, index) {
                            return (
                              <RowMenu menu={menu} key={index} />
                            );
                          ...
                          :
                          (filter === "danh-muc-san-pham" ?
                            (categoryies && categoryies.length > 0 && categoryies.map(function (menu, index) {
                              return (
                                <RowMenu menu={menu} key={index} />
                              );
                            }))
                            :
                            (filter === "thuong-hieu" ?
                              (brands && brands.length > 0 && brands.map(function (menu, index) {
                                return (
                                  <RowMenu menu={menu} key={index} />
                                );
                              }))
                              :
                              (filter === "chu-de-bai-viet" ?
                                (topics && topics.length > 0 && topics.map(function (menu, index) {
                                  return (
                                    <RowMenu menu={menu} key={index} />
                                  );
                                }))
                                :
                                (filter === "trang-don" ?
                                  (pages && pages.length > 0 && pages.map(function (menu, index) {
                                    return (
                                      <RowMenu menu={menu} key={index} />
                                    );
                                  }))
                                  :
                                  (options && options.length > 0 && options.map(function (menu, index) {
                                    return (
                                      <RowMenu menu={menu} key={index} />
                                    );
                                  }))

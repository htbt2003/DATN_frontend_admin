...
function BrandList() {
  ...
  useEffect(function () {
    (async function () {
      setLoad(true)
      const result = await BrandServices.getAll(page, condition);
  ...
  function handleStatus(item) {
    swal({
      title: "Cảnh báo",
      text: item.status == 1 ? "Tạm thời ngưng hiển thị?" : "Mở đăng lại lại này?",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
      ...
      if (result) {
        try {
          const result = await BrandServices.changeStatus(item.id)
          setReLoad(Date.now)
        }
        catch (error) {
          console.log(error)
  ...
  const handleAction = () => {
    ...
    swal({
      title: "Cảnh báo",
      text: "Chuyển vào tất cả vào thùng rác?",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
      ...
      if (result) {
        if (action === "trash") {
          try {
            const result = await BrandServices.action_trash(data);
            setReLoad(Date.now)
            console.log(true)
          }
          catch (error) {

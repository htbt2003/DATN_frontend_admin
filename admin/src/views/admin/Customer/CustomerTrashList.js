...
function CustomerTrashList() {
  ...
  document.title = "Danh sách khách hàng rác";
  var condition = {
    keySearch: keySearch,
  }
    ...
    function itemDelete(id) {
      swal({
        ...
        if (result) {
          try {
            const result = await CustomerServices.delete(id)
            setReLoad(id)
          }
          catch (error) {
            console.log(error)
    ...
    function handleStatus(item) {
      swal({
        ...
        if (result) {
          try {
            const result = await CustomerServices.changeStatus(item.id)
            setReLoad(Date.now)
          }
          catch (error) {
            console.log(error)
    ...
    const handleAction = () => {
      ...
      swal({
        ...
        if (result) {
          if (action === "trash") {
            try {
              const result = await CustomerServices.action_trash(data);
              setReLoad(Date.now)
              console.log(true)
            }
            catch (error) {

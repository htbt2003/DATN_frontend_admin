...
function PageList() {
  ...
  var condition = {
    ...
    useEffect(function(){
      (async function(){
        setLoad(true)
        const result = await PageServices.getAll(page, condition);
    ...
    function itemDelete(id) {
      swal({
        ...
        if (result) {
          try {
            const result = await PageServices.delete(id)
            setReLoad(id)
          }
          catch (error) {
            console.log(error)
    ...
    function handleStatus(item) {
      swal({
        ...
        if (result) {
          try {
            const result = await PageServices.changeStatus(item.id)
            setReLoad(Date.now)
          }
          catch (error) {
            console.log(error)
    ...
    const handleAction = () => {
      ...
      swal({
        ...
        if (result) {
          if (action === "trash") {
            try {
              const result = await PageServices.action_trash(data);
              setReLoad(Date.now)
              console.log(true)
            }
            catch (error) {

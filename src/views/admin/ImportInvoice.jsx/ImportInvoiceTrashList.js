...
function ImportInvoiceTrashList() {
  ...
  document.title = "Danh sách hóa đơn nhập rác";
  var condition = {
    keySearch: keySearch,
    date:date
  }
  useEffect(function () {
    (async function () {
      setLoad(true)
      const result = await ImportInvoiceServices.trash(page, condition);
      setImportInvoices(result.importInvoices.data)
      setTotal(result.total);
      setTrash(result.importInvoices.total)
      setLoad(false)
    })();
  }, [reLoad, page])
  function itemDelete(id) {
    swal({
      title: "Cảnh báo",
      text: "Lưu ý chuyển hóa đơn này vào thùng rác sẽ ảnh hưởng đến dữ liệu sản phẩm đang bán và báo cáo doanh thu, không thể khoi phục?",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
      ...
      if (result) {
        try {
          const result = await ImportInvoiceServices.remove(id)
          setReLoad(id)
        }
        catch (error) {
          console.log(error)
  ...
  const handleAction = () => {
    ...
    swal({
      title: "Cảnh báo",
      text: "Lưu ý những chuyển hóa đơn này vào thùng rác sẽ ảnh hưởng đến dữ liệu sản phẩm đang bán và báo cáo doanh thu, không thể khôi phục?",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
      ...
      if (result) {
        if (action === "trash") {
          try {
            const result = await ImportInvoiceServices.action_destroy(data);
            setReLoad(Date.now)
            console.log(true)
          }
          catch (error) {
  ...
  const RowMenu = ({ item, index }) => {
    return (
     ...
     checked:border-none checked:text-white hover:cursor-pointer dark:border-white/10 checked:bg-brand-500 dark:checked:bg-brand-400"
          ...
          />
          <div className="flex items-center justify-center rounded-xl" >{item.created_at}</div>
        </td>
...
export default ImportInvoiceTrashList;
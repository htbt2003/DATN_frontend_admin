...
function AttributeValueList() {
  ...
  useEffect(function () {
    (async function () {
      setLoad(true)
      const result = await AttributeValueServices.getAll(page, condition);
      setAttributes(result.attributeValues.data)
  ...
  function AttributeStore(event) {
    ...
    banner.append("attribute_id", id)
    AttributeValueServices.create(banner)
  ...
  function itemDelete(id) {
    swal({
      ...
      if (result) {
        try {
          const result = await AttributeValueServices.delete(id)
          setReLoad(id)
        }
        catch (error) {
          console.log(error)
  ...
  function handleStatus(item) {
    swal({
      ...
      if (result) {
        try {
          const result = await AttributeValueServices.changeStatus(item.id)
          setReLoad(Date.now)
        }
        catch (error) {
          console.log(error)
  ...
  const handleAction = () => {
    ...
    swal({
      ...
      if (result) {
        if (action === "trash") {
          try {
            const result = await AttributeValueServices.action_trash(data);
            setReLoad(Date.now)
            console.log(true)
          }
          catch (error) {
    ...
    return (
     ...
     checked:border-none checked:text-white hover:cursor-pointer dark:border-white/10 checked:bg-brand-500 dark:checked:bg-brand-400"
        ...
        <td className="pt-[14px] pb-[16px] sm:text-[14px] text-sm font-bold text-navy-700 dark:text-white relative">
          <div className=''>
            <Link className='text-[#078ae8]'>Cấu hình giá trị thuộc tính</Link>
      ...
      <div className='mt-5'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
          ...
          <div className="bg-white p-5 rounded-xl">
            <strong className='text-[20px]'>Thêm giá trị</strong>

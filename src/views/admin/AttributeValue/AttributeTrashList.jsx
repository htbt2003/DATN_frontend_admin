...
function AttributeTrashList() {
  ...
  useEffect(function () {
    (async function () {
      setLoad(true)
      const result = await AttributeServices.trash(page, condition);
      setAttributes(result.attributes.data)
      setTotal(result.total);
      setTrash(result.attributes.total)
      setLoad(false)
    })();
  }, [reLoad, page])
  ...
  function AttributeStore(event) {
    ...
    AttributeServices.create(banner)
      .then(function (result) {
        ...
        navigator("/admin/attribute", { replace: true })
      });
  }
  ...
  function itemDelete(id) {
    swal({
      ...
      if (result) {
        try {
          const result = await AttributeServices.remove(id)
          setReLoad(id)
        }
        catch (error) {
          console.log(error)
  ...
  function handleReStore(item) {
    swal({
      ...
      if (result) {
        try {
          const result = await AttributeServices.restore(item.id)
          setReLoad(Date.now)
        }
        catch (error) {
          console.log(error)
     ...
     checked:border-none checked:text-white hover:cursor-pointer dark:border-white/10 checked:bg-brand-500 dark:checked:bg-brand-400"
        ...
        <td className="pt-[14px] pb-[16px] sm:text-[14px] text-sm font-bold text-navy-700 dark:text-white relative">
          <div className=''>
            ...
            <button onClick={() => handleReStore(item)} className="text-lime-500 mr-2">
                <TbRestore/>
              </button>

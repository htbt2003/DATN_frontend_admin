...
function CategoryTrashList() {
  ...
  document.title = "Danh sách damh mục";
  var condition = {
    keySearch: keySearch,
  }
  ...
  useEffect(function () {
    (async function () {
      setLoad(true)
      const result = await CategoryServices.trash(page, condition);
      setCategories(result.categories.data);
      settotalTrash(result.categories.total)
  ...
  function handleStatus(item) {
    swal({
      ...
      if (result) {
        try {
          const result = await CategoryServices.restore(item.id)
          setReLoad(Date.now)
        }
        catch (error) {
          console.log(error)
  ...
  //------------pagination-------------
  const numberPage = Math.ceil(totalTrash / 5);
  ...
  const handleAction = () => {
    ...
    swal({
      ...
      if (result) {
        if (action === "trash") {
          try {
            const result = await CategoryServices.action_destroy(data);
            setReLoad(Date.now)
            console.log(true)
          }
          catch (error) {
  ...
  const RowMenu = ({ item, index }) => {
   ...
   checked:border-none checked:text-white hover:cursor-pointer dark:border-white/10 checked:bg-brand-500 dark:checked:bg-brand-400"
          ...
          />
          <img className="flex h-28 w-24 items-center justify-center rounded-xl" src={urlImage + "category/" + item.image} alt="hinh" />
          </td>
        <td className="pt-[14px] pb-[16px] sm:text-[14px] text-sm font-bold text-navy-700 dark:text-white relative">
          <div className=''>
            ...
            <button onClick={() => handleStatus(item)} className='mr-2'>
              ...
              <Link to={"/admin/category/update/" + item.id} className="text-blue-500 mr-2">
                <FaEdit />
              </Link>
          ...
          <div className='ml-auto items-center justify-center'>
            <nav aria-label="breadcrumb">
              <ol className="breadcrumb flex" >
                <li className="breadcrumb-item mr-4 text-blue-500"><Link to="/admin/category">Tất cả({total})</Link></li>
                <li className="breadcrumb-item mr-4 " aria-current="page" >Xuất bản({publish})</li>
                <li className="breadcrumb-item text-blue-500" aria-current="page"><Link to="/admin/category/trash">Rác({trash})</Link></li>
              </ol>
            </nav>
          </div>
        </div>

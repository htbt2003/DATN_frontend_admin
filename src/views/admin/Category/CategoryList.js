...
function CategoryList() {
  ...
  useEffect(function () {
    (async function () {
      setLoad(true)
      const result = await CategoryServices.getAll(page, condition);
  ...
  function handleStatus(item) {
    swal({
      ...
      if (result) {
        try {
          const result = await CategoryServices.changeStatus(item.id)
          setReLoad(Date.now)
        }
        catch (error) {
          console.log(error)
  ...
  const handleAction = () => {
    ...
    swal({
      ...
      if (result) {
        if (action === "trash") {
          try {
            const result = await CategoryServices.action_trash(data);
            setReLoad(Date.now)
            console.log(true)
          }
          catch (error) {

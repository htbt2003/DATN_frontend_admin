...
function ProductTrashList() {
  ...
  const fetchAPI = async () => {
    try {
      ...
      const result = await ProductServices.trash(page, condition);
      ...
      settotalTrash(result.products.total)
  ...
  function productDelete(id) {
    swal({
      title: "Cảnh báo",
      text: "Nếu xóa sản phẩm này, thì không thể khôi phục và dữ liệu liên quan cũng bị xóa",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
  ...
  function handleStatus(product) {
    swal({
      title: "Cảnh báo",
      text: "Khôi phục lại sản phẩm này?",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
      ...
      if (result) {
        try {
          const result = await ProductServices.restore(product.id)
          setReLoad(Date.now)
        }
        catch (error) {
          console.log(error)
  ...
  const handleAction = () => {
    ...
    swal({
      title: "Cảnh báo",
      text: "Nếu xóa những sản phẩm này, thì không thể khôi phục và những dữ liệu liên quan cũng bị xóa",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
      ...
      if (result) {
        if (action === "trash") {
          try {
            const result = await ProductServices.action_destroy(data);
            setReLoad(Date.now)
            console.log(true)
          }
          catch (error) {

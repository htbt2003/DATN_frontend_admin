...
function OrderTrashList() {
  ...
  document.title = "Danh sách đơn hàng rác";
  var condition = {
    keySearch: keySearch,
    status: status,
  }
  ...
  useEffect(function () {
    (async function () {
      setLoad(true)
      const result = await OrderServices.trash(page, condition);
  ...
  function itemDelete(id) {
    swal({
      title: "Cảnh báo",
      text: "Nếu xóa danh mục này, thì không thể khôi phục và dữ liệu liên quan cũng bị xóa, nó liên quan đến thống kê báo cáo",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
...
console.log(orders)
  ...
  function handleStatus(item) {
    swal({
      ...
      if (result) {
        try {
          const result = await OrderServices.restore(item.id)
          setReLoad(Date.now)
        }
        catch (error) {
          console.log(error)
  ...
  //------------pagination-------------
  const numberPage = Math.ceil(orders.length>0 ? orders.length / 5 : 0);
  ...
  const handleAction = () => {
    ...
    swal({
      title: "Cảnh báo",
      text: "Nếu xóa những mục này, thì không thể khôi phục và dữ liệu liên quan cũng bị xóa, nó liên quan đến thống kê báo cáo!",
      icon: "warning",
      buttons: {
        cancel: "Không",
        confirm: {
      ...
      if (result) {
        if (action === "trash") {
          try {
            const result = await OrderServices.action_destroy(data);
            setReLoad(Date.now)
            console.log(true)
          }
          catch (error) {

...
function TopicList() {
  ...
  var condition = {
    ...
    useEffect(function(){
      (async function(){
        setLoad(true)
        const result = await TopicServices.getAll(page, condition);
    ...
    function handleStatus(item) {
      swal({
        ...
        if (result) {
          try {
            const result = await TopicServices.changeStatus(item.id)
            setReLoad(Date.now)
          }
          catch (error) {
            console.log(error)
    ...
    const handleAction = () => {
      ...
      swal({
        ...
        if (result) {
          if (action === "trash") {
            try {
              const result = await TopicServices.action_trash(data);
              setReLoad(Date.now)
              console.log(true)
            }
            catch (error) {

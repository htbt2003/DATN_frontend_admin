...
function PostTrashList() {
  ...
  document.title = "Danh sách trang đơn";
  var condition = {
    keySearch: keySearch,
  }
    ...
    useEffect(function(){
      (async function(){
        setLoad(true)
        const result = await PostServices.trash(page, condition);
    ...
    function handleStatus(item) {
      swal({
        ...
        if (result) {
          try {
            const result = await PostServices.restore(item.id)
            setReLoad(Date.now)
          }
          catch (error) {
            console.log(error)
    ...
    //------------pagination-------------
    const numberPage = Math.ceil(posts.length>0 ? posts.length / 5 : 0);
    ...
    const handleAction = () => {
      ...
      swal({
        ...
        if (result) {
          if (action === "trash") {
            try {
              const result = await PostServices.action_destroy(data);
              setReLoad(Date.now)
              console.log(true)
            }
            catch (error) {
    ...
    const RowMenu = ({ item, index }) => {
     ...
     checked:border-none checked:text-white hover:cursor-pointer dark:border-white/10 checked:bg-brand-500 dark:checked:bg-brand-400"
            ...
            />
          <img className="flex h-28 w-24 items-center justify-center rounded-xl" src={urlImage + "post/" + item.image} alt="hinh" />
          </td>
          <td className="pt-[14px] pb-[16px] sm:text-[14px] text-sm font-bold text-navy-700 dark:text-white relative">
            <div className=''>
              ...
              <div className="absolute top-200 right-100 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-wrap flex-row ml-2 mt-2">
                ...
                <Link to={"/admin/post/update/" + item.id} className="text-blue-500 mr-2">
                  <FaEdit />
                </Link>
            ...
            <div className='ml-auto items-center justify-center'>
              <nav aria-label="breadcrumb">
                <ol className="breadcrumb flex" >
                  <li className="breadcrumb-item mr-4 text-blue-500"><Link to="/admin/post">Tất cả({total})</Link></li>
                  <li className="breadcrumb-item mr-4 " aria-current="page" >Xuất bản({publish})</li>
                  <li className="breadcrumb-item text-blue-500" aria-current="page"><Link to="/admin/post/trash">Rác({trash})</Link></li>
                </ol>
              </nav>
            </div>
          </div>

...
function PostList() {
  ...
  var condition = {
    ...
    useEffect(function(){
      (async function(){
        setLoad(true)
        const result = await PostServices.getAll(page, condition);
    ...
    function itemDelete(id) {
      swal({
        ...
        if (result) {
          try {
            const result = await PostServices.delete(id)
            setReLoad(id)
          }
          catch (error) {
            console.log(error)
    ...
    function handleStatus(item) {
      swal({
        ...
        if (result) {
          try {
            const result = await PostServices.changeStatus(item.id)
            setReLoad(Date.now)
          }
          catch (error) {
            console.log(error)
    ...
    const handleAction = () => {
      ...
      swal({
        title: "Cảnh báo",
        text: "Chuyển vào tất cả thùng rác?",
        icon: "warning",
        buttons: {
          cancel: "Không",
          confirm: {
        ...
        if (result) {
          if (action === "trash") {
            try {
              const result = await PostServices.action_trash(data);
              setReLoad(Date.now)
              console.log(true)
            }
            catch (error) {
